...
def clean_df(df):
    ...
    df.loc[:, ('away_score')] = np.where((df.event == 'GOAL') & (df.ev_team == df.away_team.unique()[0]), 1, 0).cumsum()
    df.loc[:, ('home_score')] = np.where((df.event == 'GOAL') & (df.ev_team == df.home_team.unique()[0]), 1, 0).cumsum()
...
def dfapply_danger_area(row):
    ...
    elif ((44 <= x <= 54 and y <= 9) or (54 <= x <= 69 and y <= 22)
          or point_in_triangle((x,y), md_triangle)):
        return 2
...
def calc_distance_togoal(pbp_df):
    ...
    pbp_df.loc[:, ('distance_togoal')] = np.sqrt((87.95-abs(pbp_df.xc))**2
                                          + pbp_df.yc**2)
...
def calc_was_tied(pbp_df):
    ...
    pbp_df.loc[:, ('was_tied')] = np.where((pbp_df.event == 'GOAL') &
                                           (pbp_df.score_diff.shift(1) == 0),
                                           1, 0)
...
def calc_rush_shot(pbp_df):
    ...
    pbp_df.loc[:, ('is_rush')] = np.where((pbp_df.time_diff < 5) &
                                          (pbp_df.event.isin(['SHOT', 'MISS', 'BLOCK', 'GOAL'])) &
                                          ((pbp_df.xc.shift(1) * pbp_df.xc < 0) |
